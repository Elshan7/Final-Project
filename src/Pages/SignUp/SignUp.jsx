...
const SignUp = () => {
  ...
  const submit = (values, actions) => {
    dispatch(signUpUser(values)); // Dispatching the signup user action
  ...
  const { values, errors, handleChange, handleSubmit } = useFormik({
    ...
    validationSchema: signUpSchema, // Validation schema from your assets
    onSubmit: submit,
  });

  useEffect(() => {
    if (userInfo) {
      navigate("/login"); // Redirect to login if user is successfully created
    }
  }, [userInfo, navigate]);
